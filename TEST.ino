...
// ===== CODE =====
String inputCode = "";
const String correctCode = "10359546";

// Booleans für Zigbee
bool keypadSolved = false;
bool temperatureAlarm = true;

// ===== TEMPERATUR ALARM LIMIT =====
int temperatureSolvedCount = 0;
const int TEMPERATURE_SOLVED_LIMIT = 3;
bool temperatureLockedSolved = false;
bool lastTemperatureAlarmState = true;
...
void updateTemperatureAlarm() {
  ...
  // Wenn das Temperatur-Rätsel 3x richtig gestellt wurde,
  // bleibt es bis zum Reset dauerhaft gelöst.
  if (temperatureLockedSolved) {
  ...
  if (millis() - lastTempReadTime >= TEMP_READ_INTERVAL_MS) {
    ...
    bool newTemperatureAlarm = lastTempC > TEMP_LIMIT_C;

    // Zählen, wenn der Zustand von Alarm AN zu Alarm AUS wechselt.
    // Also: vorher falsch/Alarm, jetzt richtig/kein Alarm.
    if (lastTemperatureAlarmState == true && newTemperatureAlarm == false) {
      temperatureSolvedCount++;

      Serial.print("Temperatur richtig gestellt. Anzahl: ");
      Serial.print(temperatureSolvedCount);
      Serial.print(" / ");
      Serial.println(TEMPERATURE_SOLVED_LIMIT);

      if (temperatureSolvedCount >= TEMPERATURE_SOLVED_LIMIT) {
        temperatureLockedSolved = true;
        newTemperatureAlarm = false;

        Serial.println("Temperatur-Raetsel dauerhaft geloest bis Reset.");
      }
    }

    temperatureAlarm = newTemperatureAlarm;
    lastTemperatureAlarmState = temperatureAlarm;
  }
